...
public class ExampleAssociatePoints<T extends ImageGray<T>, TD extends TupleDesc> {
    ...
    public ExampleAssociatePoints(DetectDescribePoint<T, TD> detDesc, AssociateDescription<TD> associate,
                                  Class<T> imageType) {
    ...
    public void associate(BufferedImage imageA, BufferedImage imageB) {
        ...
        FastQueue<TD> descA = UtilFeature.createQueue(detDesc, 200);
        FastQueue<TD> descB = UtilFeature.createQueue(detDesc, 200);
    ...
    /*
     ...
     this class create the association between the two pictures
     he is useful in cases of loading match points into array for two option:
     - measuring distance.
     - tracker.
