...
public class Maping {
	...
	ModelManager<DenseMatrix64F> managerF;
	...
	static ModelMatcher<DenseMatrix64F, AssociatedPair> robustF;

	// variables of computeMatches function
	DetectDescribePoint detDesc;
	ScoreAssociation<SurfFeature> scorer;
	static AssociateDescription<SurfFeature> associate;
	static ExampleAssociatePoints<ImageFloat32, SurfFeature> findMatches;
	...
	public Maping(double distBetweenCams, double camreaLen, double width, double height) {
		...
		estimateF = FactoryMultiView.computeFundamental_1(EnumEpipolar.FUNDAMENTAL_7_LINEAR, 2);
		generateF = new GenerateEpipolarMatrix(estimateF);
		robustF = new Ransac<DenseMatrix64F, AssociatedPair>(123123, managerF, generateF, errorMetric, 6000, 0.1);
		...
		findMatches = new ExampleAssociatePoints<ImageFloat32, SurfFeature>(detDesc, associate, ImageFloat32.class);
	}
